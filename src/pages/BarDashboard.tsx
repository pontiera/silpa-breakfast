...
export default function BarDashboard() {
  ...
  const { data: allOrders = [], isLoading, refetch } = useOrders(selectedDate)
  const updateStatus = useUpdateOrderStatus()

  // Bar sees orders with drinks only
  const orders = allOrders.filter((o) => o.coffee_tea || o.juice)
    ...
    <div className="min-h-screen flex flex-col">
      <header className="bg-[#F5F0E8] border-b border-[#E8E0D4] px-4 py-3 sticky top-0 z-20">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="font-serif text-2xl font-semibold text-[#2C1810]">SILPA Bar</h1>
            <p className="text-xs text-gray-400">Coffee · Tea · Juice</p>
          </div>
        ...
        <div className="flex gap-1 overflow-x-auto scrollbar-hide">
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                {tab.label}
                {count > 0 && <span className="text-xs bg-[#EDE8DF] text-[#8B4513] px-1.5 py-0.5 rounded-full">{count}</span>}
              </button>
            )
          })}
        </div>
      ...
      <main className="flex-1 p-4">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-40 text-gray-400">ไม่มีออเดอร์เครื่องดื่มในสถานะนี้</div>
        ) : (
